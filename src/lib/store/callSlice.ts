import { CallItem, SliceCreator } from '@/types/common';

type InitialState = {
  callState: {
    isClicked: boolean;
    selectedItemArr: CallItem[];
  };
};
...
export interface CallSlice {
  ...
  resetCallState: () => void;
  selectCallBtn: ({ id, title, amount }: CallItem) => void;
  changeItemAmount: ({ id, amount }: { id: CallItem['id']; amount: CallItem['amount'] }) => void;
}
...
export const callSlice: SliceCreator<CallSlice> =
  process.env.NODE_ENV === 'development'
    ? (set) => ({
        ...
        // 요청 항목 선택
        selectCallBtn: ({ id, title, amount }: CallItem) =>
          set(
            (state) => {
            ...
            undefined,
            'callState/selectCallBtn'
          ),
        // 요청 항목 수량 변경
        changeItemAmount: ({ id, amount }: { id: CallItem['id']; amount: CallItem['amount'] }) =>
          set(
            (state) => {
              const { callState } = state;
              // 해당 항목 수량 수정 배열
