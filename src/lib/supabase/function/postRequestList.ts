...
export default async function postRequestList(tableName: number, requestList: string) {
  const insertData: InsertRequestList = { tableNum: tableName, requestList };
  const response = await supabase.from('qr-order-request-list').insert(insertData).select();
  if (response.error) {
